...
class DiskCache(OffloadCache):
    ...
    TODO: need to write a script to load models while creating an offload_files mapping
          likely by patching `PreTrainedModel._load_parameter_into_model`
    ...
    def __init__(
        self, offload_files: dict[torch.Tensor, str], device: torch.device | str
    ):
        ...
        ] = WeakValueDictionary()  # offloaded (meta) tensors -> onloaded tensors
    ...
    def __getitem__(self, key: torch.Tensor) -> torch.Tensor:
        ...
        if key not in self.onload_values:
            ...
            if key in self.update_files:
                ...
                raise ValueError()
            with safe_open(
                file_path, framework="pt", device=self.onload_device
            ) as file:
