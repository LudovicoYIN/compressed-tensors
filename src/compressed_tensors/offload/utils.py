...
def clone_to_device(value: torch.Tensor, device: torch.device | str) -> torch.Tensor:
    if isinstance(value, torch.nn.Parameter):
        if value.requires_grad:
            # warn once:
            warnings.warn(
                "Torch Offloader does not currently support parameters with gradients"
            )
...
def is_namedtuple(data):
    """
    Checks if `data` is a `namedtuple` or not. Can have false positives,
    but only if a user is trying to mimic a `namedtuple` perfectly.
    """
    return (
